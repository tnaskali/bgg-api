...
public class JaxbConfiguration {
  ...
  static class JaxbRuntimeHints implements RuntimeHintsRegistrar {
    ...
    private static final String[][] JAXB_PROXY_INTERFACES = {
        ...
        {"jakarta.xml.bind.annotation.XmlEnumValue", "org.glassfish.jaxb.core.v2.model.annotation.Locatable"},
        {"jakarta.xml.bind.annotation.adapters.XmlJavaTypeAdapter", "org.glassfish.jaxb.core.v2.model.annotation.Locatable"}
    };

    @Override
    public void registerHints(@NotNull RuntimeHints hints, ClassLoader classLoader) {
      Stream.concat(
              Arrays.stream(JAXB_MODEL_PACKAGES).flatMap(packageName -> ClasspathUtils.getClassesInPackage(packageName, classLoader)),
      ...
      Arrays.stream(JAXB_PROXY_INTERFACES)
          .forEach(item -> hints.proxies().registerJdkProxy(Arrays.stream(item).map(ReflectionUtils::getClass).toArray(Class<?>[]::new)));
    }
